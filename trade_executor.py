...
class LocalSheetManager:
    """Compatibility shim that proxies to SQLite-backed LocalDbQueue for durability."""

    def __init__(self, data_dir: str = "local_data"):
        self.data_dir = data_dir
        os.makedirs(data_dir, exist_ok=True)
        self.archive_file = os.path.join(data_dir, "local_archive.xlsx")
        self.db = LocalDbQueue(data_dir=data_dir)
        self.lock = threading.Lock()
        logger.info(f"LocalSheetManager (DB-backed) initialized: {data_dir}")

    def add_cell_update(self, row_index, column, value, update_type="cell_update"):
        self.db.add_cell_update(row_index=row_index, column=column, value=value, update_type=update_type)

    def add_archive_operation(self, row_index, row_data, columns_to_clear=None):
        self.db.add_archive_operation(row_index=row_index, row_data=row_data, columns_to_clear=columns_to_clear or [])
        # Keep local copy as a best-effort for human inspection
        try:
            self._save_to_local_archive(row_data)
        except Exception as e:
            logger.warning(f"Local archive save failed (non-fatal): {e}")

    def add_clear_operations(self, row_index, columns):
        self.db.add_clear_operations(row_index=row_index, columns=columns)

    def get_pending_count(self):
        return self.db.get_pending_count()

    def get_batch_for_processing(self, max_batch_size=20):
        return self.db.get_batch_for_processing(max_batch_size=max_batch_size)

    def mark_batch_completed(self, completed_ids):
        self.db.mark_batch_completed(completed_ids)

    def mark_batch_failed(self, failed_ids, max_retries=3):
        self.db.mark_batch_failed(failed_ids, max_retries=max_retries)
...
class GoogleSheetTradeManager:
    ...
    def __init__(self):
        ...
        # Unit test mode short-circuit to avoid external calls
        if os.getenv("UNIT_TEST_MODE") == "1":
            import types as _types
            self.worksheet = _types.SimpleNamespace(
                row_values=lambda *a, **k: [],
                batch_update=lambda *a, **k: True,
                update_cells=lambda *a, **k: True,
            )
            self.archive_worksheet = _types.SimpleNamespace(
        ...
        self.column_mapping = {}
        # Load persisted active positions (survive restarts)
        try:
            restored = self.db.get_all_active_positions()
            if restored:
                self.active_positions.update(restored)
                logger.info(f"Restored {len(restored)} active positions from DB")
        except Exception as e:
            logger.error(f"Failed to restore active positions from DB: {e}")
        ...
        try:
            ...
            logger.warning(f"Health server init failed: {e}")
        ...
        class Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                if self.path != "/health":
                    ...
                    return
                try:
                    counts = manager.local_manager.get_pending_count()
                    active = len(manager.active_positions)
                    # simple DB check
                    ok = True
                    try:
                        _ = manager.db.get_meta("ping", None)
                    except Exception:
                        ok = False
                    payload = {
                        "ok": ok,
                        ...
                        "last_reconcile_ts": manager._last_reconcile_ts,
                    }
    ...
    def _run_with_backoff(self, fn, *args, **kwargs):
        max_attempts = kwargs.pop("_max_attempts", 5)
        base = kwargs.pop("_base", 1.5)
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if status == 429 or status == 503:
                    ...
                    logger.warning(f"RATE_LIMIT_HIT attempt={attempt} sleeping {sleep_s:.2f}s")
                    time.sleep(sleep_s)
                    continue
                raise
            except (requests.exceptions.RequestException, Exception) as e:
                sleep_s = (base ** attempt) + random.uniform(0, 0.5)
                logger.warning(f"NETWORK_OR_UNKNOWN_ERROR attempt={attempt} sleeping {sleep_s:.2f}s: {e}")
                time.sleep(sleep_s)
                    ...
                    if status == "FILLED" or (status in ["CANCELED", "REJECTED", "EXPIRED"] and cumulative_quantity > 0):
                        ...
                        if symbol in self.active_positions:
                            ...
                            try:
                                ...
                                try:
                                    self.db.upsert_active_position(symbol, self.active_positions[symbol])
                                except Exception:
                                    pass
                            except Exception as e:
                        ...
                        if symbol in self.active_positions:
                            try:
                                ...
                                self.db.delete_active_position(symbol)
                            except Exception:
                                pass
    ...
    def execute_trade(self, trade_signal):
        ...
        if action == "BUY":
            ...
            try:
                ...
                try:
                    self.db.upsert_active_position(symbol, position_data)
                except Exception as e:
                    logger.error(f"Persist position failed: {e}")
                ...
                if is_filled and symbol in self.active_positions:
                    ...
                    if tp_order_id or sl_order_id:
                        ...
                        try:
                            ...
                            logger.error(f"Persist TP/SL ids failed: {e}")
        ...
        try:
            if symbol in self.active_positions:
                ...
                if order_id_to_cancel:
                    # Cancel with retries and verify
                    for attempt in range(1, 4):
                        cancel_params = {"order_id": order_id_to_cancel}
                        response = self.exchange_api.send_request("private/cancel-order", cancel_params)
                        ok = bool(response and response.get("code") == 0)
                        status = self.exchange_api.get_order_status(order_id_to_cancel)
                        if ok and status == "CANCELED":
                            logger.info(f"ORDER_CANCELLED symbol={symbol} order_id={order_id_to_cancel}")
                            try:
                                # Reflect in DB
                                position['tp_order_id'] = position.get('tp_order_id') if order_id_to_keep == position.get('tp_order_id') else None
                                position['sl_order_id'] = position.get('sl_order_id') if order_id_to_keep == position.get('sl_order_id') else None
                                self.db.upsert_active_position(symbol, position)
                            except Exception:
                                pass
                            self.telegram.send_message(
                                f"❌ Cancelled opposite order:\nSymbol: {symbol}\nOrder ID: {order_id_to_cancel}"
                            )
                            return True
                        sleep_s = 1.5 ** attempt + random.uniform(0, 0.5)
                        logger.warning(f"Cancel verify retry {attempt} for {order_id_to_cancel} sleeping {sleep_s:.2f}s (status={status})")
                        time.sleep(sleep_s)
    ...
    def _process_archive_batch(self, archives):
        ...
        try:
            ...
            # Build idempotency set from Archive sheet to avoid duplicates
            existing_keys = set()
            try:
                existing = self.archive_worksheet.get_all_records()
                for r in existing:
                    key = (str(r.get('Coin', '')).strip(), str(r.get('Order PURCHASE Date', '')).strip(), str(r.get('SOLD Date', '')).strip())
                    existing_keys.add(key)
            except Exception as e:
                logger.warning(f"Could not build archive idempotency set: {e}")
            ...
            for i, archive in enumerate(archives):
                try:
                    ...
                    dedup_key = (str(archive_data[1]).strip(), str(archive_data[11]).strip(), str(archive_data[15]).strip())
                    if dedup_key in existing_keys:
                        logger.info(f"Archive entry already exists for {dedup_key}, skipping write")
                        continue
                    ...
                    try:
                        ...
                        def do_update():
                            return self.archive_worksheet.update(range_name, [archive_data], value_input_option='USER_ENTERED')
                        result = self._run_with_backoff(do_update, _max_attempts=5)
