...
class GoogleSheetIntegration:
    ...
    def _run_with_backoff(self, fn, *args, **kwargs):
        max_attempts = kwargs.pop("_max_attempts", self._max_attempts)
        base = kwargs.pop("_base", self._base_backoff)
    ...
    def update_analysis(self, row_index, data):
        """Update analysis data using batchUpdate with backoff and idempotency checks"""
        try:
            symbol = data.get("symbol", "")
            ...
            # Build batchUpdate payload with A1 ranges
            updates = [
                (3, data["last_price"]),
                (4, data.get("buy_target", data["last_price"])),
                (18, data["rsi"]),
                (19, data["ma200"]),
                (20, "YES" if data["ma200_valid"] else "NO"),
                (21, data["resistance"]),
                (22, data["support"]),
                (23, data["timestamp"]),
                (26, data["ma50"]),
                (27, data["ema10"]),
                (28, "YES" if data["ma50_valid"] else "NO"),
                (29, "YES" if data["ema10_valid"] else "NO"),
                (31, "TradingView"),
                (32, "NO"),
                (35, data.get("volume", 0)),
                (36, data.get("volume_ratio", 0)),
                (5, data["action"] if data["action"] == "BUY" else ("WAIT" if data["action"] == "WAIT" else data["action"]))
            ]
            if data["action"] == "BUY":
                updates.append((6, data["take_profit"]))
                updates.append((7, data["stop_loss"]))

            batch_requests = []
            for col_idx, value in updates:
                a1 = f"{self._column_index_to_letter(col_idx)}{row_index}"
            ...
            if "Quota exceeded" in str(e):
                ...
                # This path should be rare now; fall back to smaller batches once
                try:
